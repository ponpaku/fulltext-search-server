...
- 2026-01-15: 改善: インデックス世代のファイルウォームアップ/アイドル時keep-warmを追加し、コールドスタート時の初回検索を低減。
  - `WARMUP_ENABLE` で世代ディレクトリの先頭/ストライド読みを実行
  - `KEEPWARM_ENABLE` でアイドル時に軽量ウォームアップを再実行
  - 任意でクエリ再生ウォームアップ（`QUERY_REPLAY_ENABLE`）を追加
